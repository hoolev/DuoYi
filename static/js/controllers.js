...
controller('userResetCtrl', ['$scope', '$routeParams',
    function ($scope, $routeParams) {
        ...
        $scope.submit = function () {
            jsGen.rootScope.global.loading = true;
            var result = jsGen.rest.user.save({
                Uid: 'reset'
            }, {
                name: $scope.name,
                email: $scope.email,
...
controller('userRegisterCtrl', ['$scope',
    function ($scope) {
...
controller('userCtrl', ['$scope', '$routeParams',
    function ($scope, $routeParams) {
        var Uid;
        $scope.isMe = false;
        if ($routeParams.UID) {
...
controller('userArticleCtrl', ['$scope',
    function ($scope) {
        $scope.$on('pagination', function (event, doc) {
            event.stopPropagation();
            doc.Uid = $scope.userOperate.Uid;
        ...
        $scope.remove = function (article) {
            jsGen.rootScope.global.loading = true;
            var result = jsGen.rest.article.remove({
                ID: article._id
            }, null, function () {
                jsGen.rootScope.global.loading = false;
                if (result.remove) {
...
controller('userEditCtrl', ['$scope',
    function ($scope) {
        ...
        $scope.verifyEmail = function () {
            jsGen.rootScope.global.loading = true;
            var verify = jsGen.rest.user.save({
                Uid: 'reset'
            }, {
                request: 'role'
            }, function () {
        ...
        $scope.submit = function () {
            ...
            if (data.email) {
                jsGen.rootScope.global.loading = true;
                changeEmail = jsGen.rest.user.save({
                    Uid: 'reset'
                }, {
                    email: data.email,
                    request: 'email'
...
controller('articleEditorCtrl', ['$scope', '$routeParams',
    function ($scope, $routeParams) {
        ...
        $scope.$watch('title', function (title) {
            ...
            if (!$scope.markdownHelp) {
                $scope.previewTitle = $scope.title || '预览';
            }
        });
        $scope.$watch('content', function (content) {
            if (typeof content !== 'string') {
        ...
        $scope.$watch('original_price', function (original_price) {
            $scope.originalPrice = parseFloat(original_price);
        });
        $scope.$watch('vip_price', function (vip_price) {
            $scope.vipPrice = parseFloat(vip_price);
            $scope.isvip = true;
        });
        $scope.wmdHelp = function (s) {
            if (s === 'preview') {
        ...
        $scope.submit = function () {
            ...
            data.tagsList = $scope.tagsList;
            data.originalPrice = $scope.originalPrice;
            if($scope.vipPrice === 0) {
                data.vipPrice = $scope.originalPrice;
            } else {
                data.vipPrice = $scope.vipPrice;
            }
            data.isvip = $scope.isvip;
            data.refer = $scope.refer;
            data._id = $scope._id;
            if ($routeParams.ID) {
                parameter = {
...
controller('adminTagCtrl', ['$scope',
    function ($scope) {
        ...
        $scope.remove = function (tag) {
            jsGen.rootScope.global.loading = true;
            var result = jsGen.rest.tag.remove({
                ID: tag._id
            }, null, function () {
                jsGen.rootScope.global.loading = false;
                if (result.remove) {
        ...
        $scope.submit = function () {
            ...
            jsGen.rootScope.global.loading = true;
            var result = jsGen.rest.tag.save({
                ID: 'admin'
            }, {
                data: data
            }, function () {
...
controller('dishesCtrl', ['$scope', '$routeParams',
    function ($scope, $routeParams) {
        var restPath = jsGen.rest.article;
        $scope.data = null;

        jsGen.rootScope.global.loading = true;
        jsGen.getUser($routeParams.name, function (result) {
            jsGen.rootScope.global.loading = false;
            if (result.err) {
                jsGen.rootScope.msg = result.err;
            } else {
                ...
                $scope.$on('pagination', function (event, doc) {
                    ...
                    doc.ID = $routeParams.OP || $routeParams.TAG || 'latest';
                    jsGen.rootScope.global.loading = true;
                    var ret = restPath.get(doc, function () {
                        jsGen.rootScope.global.loading = false;
                        if (!ret.err) {
                                $scope.data = ret.data;
                                $scope.pagination = null;
...
controller('menuCtrl', ['$scope', '$routeParams',
    function ($scope, $routeParams) {
        ...
        $scope.$watch('table_number', function (table_number) {
            $scope.tableNumber = parseFloat(table_number);
        });
        
 
        $scope.submit = function () {
            var cart = {};
            cart.tableNumber = $scope.tableNumber;
            cart.restaurant = $routeParams.name;
            cart.total = simpleCart.total();
            ...
            simpleCart.each(function(item){
                ...
                dishes[itemCount++] = data;
            });

            cart.dishes = dishes;
            var result = jsGen.rest.cart.save({},cart,function() {
                jsGen.rootScope.global.loading = false;
                if(!result.err) {
                    jsGen.cache.cart.put(result._id,result);
                    //$scope.global.cart = jsGen.union(result);
                    jsGen.location.path('/welcome');
                } else {
                    jsGen.rootScope.msg = result.err;
                }
            });
...
controller('indentCtrl', ['$scope',
    function ($scope) {
        $scope.user = $scope.global.user;
        function print(indent) {
		    LODOP=getLodop(document.getElementById('LODOP_OB'),document.getElementById('LODOP_EM'));  
		    LODOP.PRINT_INIT(indent.tableNumber);
		    LODOP.SET_PRINT_STYLE("FontSize",22);
		    LODOP.SET_PRINT_STYLE("Bold",1);
            LODOP.ADD_PRINT_TEXT(120,163,700,20,indent.restaurant+"--消费账单");
			LODOP.ADD_PRINT_BARCODE(67,480,116,114,"QRCode","http://127.0.0.1:3000/comba");
		    LODOP.SET_PRINT_STYLEA(0,"QRCodeVersion",3);
			LODOP.SET_PRINT_STYLE("FontSize",18);
		    LODOP.SET_PRINT_STYLE("Bold",0);
			LODOP.ADD_PRINT_TEXT(170,100,100,20,"桌号：");
            LODOP.ADD_PRINT_TEXT(170,162,100,20,indent.tableNumber);
           	LODOP.ADD_PRINT_TEXT(170,240,200,20,"菜单总额：");
			LODOP.ADD_PRINT_TEXT(170,350,100,20,indent.total);
			LODOP.ADD_PRINT_TEXT(170,430,200,20,"单号：");
			LODOP.ADD_PRINT_TEXT(170,500,500,20,indent.date + indent._id);
			LODOP.ADD_PRINT_LINE(195,27,195,691,0,0);
			LODOP.ADD_PRINT_TEXT(196,100,200,20,"打印时间：");
			var time=new Date();
            LODOP.ADD_PRINT_TEXT(196,208,250,20,time.getFullYear()+"/"+time.getMonth()+"/"+time.getDate()+"  "+time.getHours()+":"+time.getMinutes());
            //以上为表头，下面为菜单明细                      
            LODOP.ADD_PRINT_TEXT(250,100,1000,20,"菜名     单价     数量     金额");
			var top=280;
			indent.dishes.forEach(function (dish){
                LODOP.ADD_PRINT_TEXT(top,100,100,20,dish.name);
				LODOP.ADD_PRINT_TEXT(top,215,100,20,dish.price);
				LODOP.ADD_PRINT_TEXT(top,325,100,20,dish.quantity);
				LODOP.ADD_PRINT_TEXT(top,428,100,20,dish.total);
				top=top+28;
				
            });
			LODOP.ADD_PRINT_LINE(top+30,27,top+30,691,0,0);
			LODOP.ADD_PRINT_TEXT(top+40,100,700,20,"地址：云裳餐饮天河分店118号");
			LODOP.ADD_PRINT_TEXT(top+65,100,700,20,"电话：020-1111111111");
			LODOP.SET_PRINT_STYLE("FontSize",22);
		    LODOP.SET_PRINT_STYLE("Bold",1);
			LODOP.SET_PRINT_STYLE("Italic",1);
			LODOP.ADD_PRINT_TEXT(top+120,100,700,20,"舌尖上的美味，尽在尚品小笼分享！");

		    LODOP.PRINT();	
        };

        var result = jsGen.rest.cart.get({OP: 'index'}, function () {
            jsGen.rootScope.global.loading = false;
            if (!result.err) {
                    $scope.data = result.data;
                    $scope.pagination = null;
        ...
        $scope.submit = function (indent) {
            indent.needPrint = false;
            print(indent);
            var ret = jsGen.rest.cart.save({OP: 'admin'},indent,function() {
                jsGen.rootScope.global.loading = false;
                if(!ret.err) {
                    jsGen.cache.cart.put(ret._id,ret);
		            CreateOneFormPage();
		            LODOP.PRINT();	
                    
                } else {
                    jsGen.rootScope.msg = ret.err;
                }
            });
        };
    }
]).
controller('adminArticleCtrl', ['$scope',
    function ($scope) {

    }
]).
controller('adminGlobalCtrl', ['$scope',
    function ($scope) {
        var originData = {};
        jsGen.rootScope.global.loading = true;
        $scope.global = jsGen.rest.index.get({
            OP: 'admin'
        }, function () {
            jsGen.rootScope.global.loading = false;
        ...
        $scope.submit = function () {
            ...
            jsGen.rootScope.global.loading = true;
            var result = jsGen.rest.index.save({
                OP: 'admin'
            }, data, function () {
                jsGen.rootScope.global.loading = false;
                if (!result.err) {
